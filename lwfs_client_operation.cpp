...
int lwfsClient::fuse_read(const char *path, char *buf, size_t size,
...
int lwfsClient::fuse_write(const char *path, const char *buf, size_t size,
                           off_t offset, struct fuse_file_info *fi) {
  operation send_msg;
  strcpy(send_msg.file_path, path);
  send_msg.offset = offset;
...
int lwfsClient::fuse_readdir(const char *path, void *buf,
...
int lwfsClient::fuse_setxattr(const char *path, const char *key,
...
int lwfsClient::fuse_getxattr(const char *path, const char *key, char *value,
                              size_t size) {
  operation send_msg;
  strcpy(send_msg.file_path, path);
...
// 这个地方有坑，linux
// posix接口readlink返回值是读出的buf长度，而fuse让成功时返回0
...
int lwfsClient::fuse_fsync(const char *path, int fd,
                           struct fuse_file_info *fi) {
